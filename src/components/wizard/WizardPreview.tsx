  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <button onClick={onBack} className="rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">
          Retour
        </button>
        <div className="flex gap-3">
          <button
            onClick={async () => { await saveWedding(); addEvent() }}
            className="rounded-lg border border-[#d4af37] bg-white px-4 py-2 text-sm font-medium text-[#d4af37] hover:bg-[#faf6f1]"
          >
            + Ajouter un autre événement
          </button>
          <button
            onClick={async () => { await saveWedding(); setSaved(true) }}
            className="rounded-lg bg-[#1a3c34] px-6 py-2 text-sm font-medium text-white hover:bg-[#2a4c44] disabled:opacity-40"
            disabled={saved}
          >
            {saved ? 'Sauvegardé ✓' : 'Terminer'}
          </button>
        </div>
      </div>
            ...
            {(['jours', 'heures', 'minutes', 'secondes'] as const).map((label, i) => {
              ...
              return (
                <div key={label} className="rounded-lg p-4" style={{ backgroundColor: palette.accent }}>
                  <span className="block font-serif text-3xl font-light" style={{ color: palette.primary }}>{value}</span>
                  <span className="mt-1 block text-xs uppercase tracking-widest" style={{ color: palette.text }}>{label}</span>
                </div>
              )
            })}
          </div>
            ...
            <div className="flex-1">
              ...
              <div className="mt-2 flex items-center gap-2">
                <input
                  readOnly
                  value={shareUrl}
                  className="flex-1 rounded-lg border border-gray-200 bg-gray-50 p-2 text-sm text-gray-600 outline-none"
                />
                <button
                  onClick={handleCopy}
